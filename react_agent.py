...
def aql_query_agent(state: AgentState):
    ...
    try:
        ...
        if state["messages"] and isinstance(state["messages"][-1], AIMessage):
            state["internal_state"]["last_aql_agent_response"] = state["messages"][-1].content
        else:
        ...
        state["internal_state"]["aql_agent_error"] = str(e)
...
def run_agent_test(agent_func, query: str):
    ...
    # Get the response - first try from the callback
    response = callback.get_final_response()
    
    # If no response from callback, try from the messages
    ...
    if not response:
        if agent_func.__name__ == "patient_data_agent" and "last_patient_agent_response" in result_state["internal_state"]:
            response = result_state["internal_state"]["last_patient_agent_response"]
        elif agent_func.__name__ == "population_health_agent" and "last_population_agent_response" in result_state["internal_state"]:
            response = result_state["internal_state"]["last_population_agent_response"]
        elif agent_func.__name__ == "aql_query_agent" and "last_aql_agent_response" in result_state["internal_state"]:
            response = result_state["internal_state"]["last_aql_agent_response"]
        elif agent_func.__name__ == "graph_analysis_agent" and "last_graph_agent_response" in result_state["internal_state"]:
            response = result_state["internal_state"]["last_graph_agent_response"]
...
def test_all_agents():
    ...
    for query in test_queries["patient_data_agent"]:
        ...
        # Run the agent
        result_state = patient_data_agent(state)
        
        # Get the response directly from the callback
        response = callback.get_final_response()
    ...
    for query in test_queries["population_health_agent"]:
        ...
        # Run the agent
        result_state = population_health_agent(state)
